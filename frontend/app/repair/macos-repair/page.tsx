...
interface MacOSRepair {
  id: string;
  hostname: string;
  ipAddress: string;
  issueType: 'disk' | 'memory' | 'cpu' | 'network' | 'service' | 'permission';
...
export default function MacOSRepairPage() {
  const [repairs, setRepairs] = useState<MacOSRepair[]>([]);
  ...
  const loadRepairs = async () => {
    ...
    setError(null);
    try {
      const resp = await api.get('/api/v1/repair/macos');
      ...
      setRepairs(
        items.map((item: any) => ({
          ...
          issueType: (item.issue_type || 'service') as MacOSRepair['issueType'],
          severity: (item.severity || 'low') as MacOSRepair['severity'],
          status: (item.status || 'detected') as MacOSRepair['status'],
  ...
  const handleRepair = async (repairId: string) => {
    try {
      await api.post(`/api/v1/repair/macos/${repairId}/repair`);
      await loadRepairs();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">macOS修复</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>macOS主机修复任务</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
