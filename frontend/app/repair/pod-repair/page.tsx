...
interface PodRepair {
  ...
  podName: string;
  nodeName: string;
  issueType: 'crash' | 'oom' | 'evicted' | 'pending' | 'image' | 'network';
...
export default function PodRepairPage() {
  const [repairs, setRepairs] = useState<PodRepair[]>([]);
  ...
  const loadRepairs = async () => {
    ...
    setError(null);
    try {
      const resp = await api.get('/api/v1/repair/pod');
      ...
      setRepairs(
        items.map((item: any) => ({
          ...
          issueType: (item.issue_type || 'crash') as PodRepair['issueType'],
          severity: (item.severity || 'low') as PodRepair['severity'],
          status: (item.status || 'detected') as PodRepair['status'],
  ...
  const handleRepair = async (repairId: string) => {
    try {
      await api.post(`/api/v1/repair/pod/${repairId}/repair`);
      await loadRepairs();
    } catch (err: any) {
  ...
  const getIssueTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'evicted': return '驱逐';
      case 'pending': return '挂起';
  ...
  const filteredRepairs = repairs.filter((repair) => {
    ...
    const matchesSearch = repair.podName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Pod修复</h1>
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            <Select value={filterIssueType} onValueChange={setFilterIssueType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="oom">内存溢出</SelectItem>
                <SelectItem value="evicted">驱逐</SelectItem>
              ...
              <TableBody>
                {filteredRepairs.map((repair) => (
                  <TableRow key={repair.id}>
                    ...
                    <TableCell className="font-medium">{repair.podName}</TableCell>
                    <TableCell>{repair.nodeName}</TableCell>
