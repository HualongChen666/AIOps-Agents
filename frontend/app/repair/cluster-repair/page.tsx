...
interface ClusterRepair {
  id: string;
  clusterName: string;
  clusterType: 'kubernetes' | 'docker-swarm' | 'nomad' | 'mesos';
  issueType: 'node' | 'network' | 'storage' | 'control-plane' | 'etcd' | 'dns';
...
export default function ClusterRepairPage() {
  const [repairs, setRepairs] = useState<ClusterRepair[]>([]);
  ...
  const loadRepairs = async () => {
    ...
    setError(null);
    try {
      const resp = await api.get('/api/v1/repair/cluster');
      const items = resp.data?.items || [];
      setRepairs(
        items.map((item: any) => ({
          id: item.id || String(Date.now()),
          clusterName: item.cluster_name || '',
          clusterType: (item.cluster_type || 'kubernetes') as ClusterRepair['clusterType'],
          issueType: (item.issue_type || 'node') as ClusterRepair['issueType'],
          severity: (item.severity || 'low') as ClusterRepair['severity'],
          status: (item.status || 'detected') as ClusterRepair['status'],
  ...
  const handleRepair = async (repairId: string) => {
    try {
      await api.post(`/api/v1/repair/cluster/${repairId}/repair`);
      await loadRepairs();
    } catch (err: any) {
  ...
  const getIssueTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'control-plane': return '控制平面';
  ...
  const filteredRepairs = repairs.filter((repair) => {
    ...
    const matchesSearch = repair.clusterName.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">集群修复</h1>
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            <Select value={filterIssueType} onValueChange={setFilterIssueType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="control-plane">控制平面</SelectItem>
              ...
              <TableBody>
                {filteredRepairs.map((repair) => (
                  <TableRow key={repair.id}>
                    ...
                    <TableCell className="font-medium">{repair.clusterName}</TableCell>
                    <TableCell>{repair.clusterType}</TableCell>
