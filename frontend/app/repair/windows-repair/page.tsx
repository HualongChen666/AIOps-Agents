...
interface WindowsRepair {
  id: string;
  hostname: string;
  ipAddress: string;
  issueType: 'registry' | 'service' | 'disk' | 'memory' | 'network' | 'update';
...
export default function WindowsRepairPage() {
  const [repairs, setRepairs] = useState<WindowsRepair[]>([]);
  ...
  const loadRepairs = async () => {
    ...
    setError(null);
    try {
      const resp = await api.get('/api/v1/repair/windows');
      ...
      setRepairs(
        items.map((item: any) => ({
          ...
          issueType: (item.issue_type || 'service') as WindowsRepair['issueType'],
          severity: (item.severity || 'low') as WindowsRepair['severity'],
          status: (item.status || 'detected') as WindowsRepair['status'],
      ...
      console.error('加载Windows修复失败:', err);
  ...
  const handleRepair = async (repairId: string) => {
    try {
      await api.post(`/api/v1/repair/windows/${repairId}/repair`);
      await loadRepairs();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Windows修复</h1>
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            <Select value={filterIssueType} onValueChange={setFilterIssueType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="registry">注册表</SelectItem>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Windows主机修复任务</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
