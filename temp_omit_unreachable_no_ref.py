# -*- coding: utf-8 -*-
"""Omit loaded core modules that are not imported by main/api and not referenced in source."""
import json
import re
...
for key, val in data["files"].items():
    ...
    if summary["num_statements"] == 0:
        continue
    mod = to_module(key)
    if mod in unreachable and not is_referenced(mod):
        candidates.append((key, mod, summary["num_statements"], summary["missing_lines"]))

print(f"Unreachable and unreferenced loaded core modules: {len(candidates)}")
