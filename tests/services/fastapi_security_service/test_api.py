# -*- coding: utf-8 -*-
"""API tests for the FastAPI Security microservice."""
...
from services.fastapi_security_service import config
from services.fastapi_security_service import main_app as main_module
from services.fastapi_security_service.config import settings
from services.fastapi_security_service.main_app import app
from services.fastapi_security_service.metrics import MetricsCollector
from services.fastapi_security_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"fastapi_security_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/fastapi-security/{method.replace('_', '-')}",
                json=payload,
            )
