# -*- coding: utf-8 -*-
"""API tests for the Message Queue microservice."""
...
from services.message_queue_service import config
from services.message_queue_service import main_app as main_module
from services.message_queue_service.config import settings
from services.message_queue_service.main_app import app
from services.message_queue_service.metrics import MetricsCollector
from services.message_queue_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"message_queue_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/message-queue/{method.replace('_', '-')}",
                json=payload,
            )
