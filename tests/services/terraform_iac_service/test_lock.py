# -*- coding: utf-8 -*-
"""Lock and idempotency tests for the Terraform IaC microservice."""
...
from services.terraform_iac_service.lock import LockManager
...
async def test_idempotency_manager():
    metrics = MetricsCollector(f"terraform_iac_idemp_{uuid.uuid4().hex[:6]}")
...
async def test_service_idempotent_request():
    metrics = MetricsCollector(f"terraform_iac_idemp_req_{uuid.uuid4().hex[:6]}")
    service = Service(redis_url="", metrics=metrics)
    op = OPERATIONS[0]
