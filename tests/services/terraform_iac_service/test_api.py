# -*- coding: utf-8 -*-
"""API tests for the Terraform IaC microservice."""
...
from services.terraform_iac_service import config
from services.terraform_iac_service import main_app as main_module
from services.terraform_iac_service.config import settings
from services.terraform_iac_service.main_app import app
from services.terraform_iac_service.metrics import MetricsCollector
from services.terraform_iac_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"terraform_iac_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/terraform-iac/{method.replace('_', '-')}",
                json=payload,
            )
