# -*- coding: utf-8 -*-
"""API tests for the Automated Operations microservice."""
...
from services.automated_ops_service import config
from services.automated_ops_service import main_app as main_module
from services.automated_ops_service.config import settings
from services.automated_ops_service.main_app import app
from services.automated_ops_service.metrics import MetricsCollector
from services.automated_ops_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"automated_ops_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/automated-ops/{method.replace('_', '-')}",
                json=payload,
            )
