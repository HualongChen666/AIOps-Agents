# -*- coding: utf-8 -*-
"""API tests for the Incident Runbook microservice."""
...
from services.incident_runbook_service.config import settings
from services.incident_runbook_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"incident_runbook_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/incident-runbook/{method.replace('_', '-')}",
                json=payload,
            )
