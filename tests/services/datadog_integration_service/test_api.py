# -*- coding: utf-8 -*-
"""API tests for the Datadog Integration microservice."""
...
from services.datadog_integration_service import config
from services.datadog_integration_service import main_app as main_module
from services.datadog_integration_service.config import settings
from services.datadog_integration_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"datadog_integration_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/datadog-integration/{method.replace('_', '-')}",
                json=payload,
            )
