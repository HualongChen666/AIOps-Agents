# -*- coding: utf-8 -*-
"""gRPC client/server tests for the Datadog Integration microservice."""
...
from services.datadog_integration_service.grpc.client import DatadogIntegrationServiceRPCClient
from services.datadog_integration_service.grpc.server import DatadogIntegrationServiceRPCServer


@pytest.mark.asyncio
async def test_rpc_server_register_and_call():
    server = DatadogIntegrationServiceRPCServer()
...
async def test_rpc_client_call():
    client = DatadogIntegrationServiceRPCClient(base_url="http://test")
