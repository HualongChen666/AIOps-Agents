# -*- coding: utf-8 -*-
"""API tests for the Backup Recovery Drill microservice."""
...
from services.backup_recovery_drill_service import config
from services.backup_recovery_drill_service import main_app as main_module
from services.backup_recovery_drill_service.config import settings
from services.backup_recovery_drill_service.main_app import app
from services.backup_recovery_drill_service.metrics import MetricsCollector
from services.backup_recovery_drill_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"backup_recovery_drill_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/backup-recovery-drill/{method.replace('_', '-')}",
                json=payload,
            )
