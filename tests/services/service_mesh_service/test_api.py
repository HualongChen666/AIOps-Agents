# -*- coding: utf-8 -*-
"""API tests for the Service Mesh microservice."""
...
from services.service_mesh_service import config
from services.service_mesh_service import main_app as main_module
from services.service_mesh_service.config import settings
from services.service_mesh_service.main_app import app
from services.service_mesh_service.metrics import MetricsCollector
from services.service_mesh_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"service_mesh_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/service-mesh/{method.replace('_', '-')}",
                json=payload,
            )
