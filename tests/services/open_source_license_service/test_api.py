# -*- coding: utf-8 -*-
"""API tests for the Open Source License microservice."""
...
from services.open_source_license_service import config
from services.open_source_license_service import main_app as main_module
from services.open_source_license_service.config import settings
from services.open_source_license_service.main_app import app
from services.open_source_license_service.metrics import MetricsCollector
from services.open_source_license_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"open_source_license_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/open-source-license/{method.replace('_', '-')}",
                json=payload,
            )
