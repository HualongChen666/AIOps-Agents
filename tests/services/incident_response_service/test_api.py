# -*- coding: utf-8 -*-
"""API tests for the Incident Response microservice."""
...
from services.incident_response_service import config
from services.incident_response_service import main_app as main_module
from services.incident_response_service.config import settings
from services.incident_response_service.main_app import app
from services.incident_response_service.metrics import MetricsCollector
from services.incident_response_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"incident_response_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/incident-response/{method.replace('_', '-')}",
                json=payload,
            )
