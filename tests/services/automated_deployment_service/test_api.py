# -*- coding: utf-8 -*-
"""API tests for the Automated Deployment microservice."""
...
from services.automated_deployment_service import config
from services.automated_deployment_service import main_app as main_module
from services.automated_deployment_service.config import settings
from services.automated_deployment_service.main_app import app
from services.automated_deployment_service.metrics import MetricsCollector
from services.automated_deployment_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"automated_deployment_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/automated-deployment/{method.replace('_', '-')}",
                json=payload,
            )
