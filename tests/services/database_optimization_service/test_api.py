# -*- coding: utf-8 -*-
"""API tests for the Database Optimization microservice."""
...
from services.database_optimization_service import config
from services.database_optimization_service import main_app as main_module
from services.database_optimization_service.config import settings
from services.database_optimization_service.main_app import app
from services.database_optimization_service.metrics import MetricsCollector
from services.database_optimization_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"database_optimization_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/database-optimization/{method.replace('_', '-')}",
                json=payload,
            )
