# -*- coding: utf-8 -*-
"""API tests for the Ansible Automation microservice."""
...
from services.ansible_automation_service import config
from services.ansible_automation_service import main_app as main_module
from services.ansible_automation_service.config import settings
from services.ansible_automation_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"ansible_automation_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/ansible-automation/{method.replace('_', '-')}",
                json=payload,
            )
