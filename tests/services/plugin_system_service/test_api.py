# -*- coding: utf-8 -*-
"""API tests for the Plugin System microservice."""
...
from services.plugin_system_service import config
from services.plugin_system_service import main_app as main_module
from services.plugin_system_service.config import settings
from services.plugin_system_service.main_app import app
from services.plugin_system_service.metrics import MetricsCollector
from services.plugin_system_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"plugin_system_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/plugin-system/{method.replace('_', '-')}",
                json=payload,
            )
