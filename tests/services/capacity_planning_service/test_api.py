# -*- coding: utf-8 -*-
"""API tests for the Capacity Planning microservice."""
...
from services.capacity_planning_service.config import settings
from services.capacity_planning_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"capacity_planning_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/capacity-planning/{method.replace('_', '-')}",
                json=payload,
            )
