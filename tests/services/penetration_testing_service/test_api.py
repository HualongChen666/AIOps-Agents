# -*- coding: utf-8 -*-
"""API tests for the Penetration Testing microservice."""
...
from services.penetration_testing_service import config
from services.penetration_testing_service import main_app as main_module
from services.penetration_testing_service.config import settings
from services.penetration_testing_service.main_app import app
from services.penetration_testing_service.metrics import MetricsCollector
from services.penetration_testing_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"penetration_testing_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/penetration-testing/{method.replace('_', '-')}",
                json=payload,
            )
