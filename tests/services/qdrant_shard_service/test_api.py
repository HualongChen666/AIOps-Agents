# -*- coding: utf-8 -*-
"""API tests for the Qdrant shard cluster microservice."""
...
from services.qdrant_shard_service import main_app as main_module
from services.qdrant_shard_service.main_app import app
...
async def reset_service():
    from services.qdrant_shard_service import config

    config.settings.redis_url = ""
    config.settings.database_url = ""
    config.settings.qdrant_url = ""
    metrics = MetricsCollector("qdrant_shard_api_test")
...
async def test_metrics():
    ...
    assert response.status_code == 200
    assert "qdrant" in response.text or "shard" in response.text
