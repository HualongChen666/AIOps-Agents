# -*- coding: utf-8 -*-
"""补充 Qdrant shard cluster 核心分支覆盖率测试。"""
...
import services.qdrant_shard_service.cache as cache_module
import services.qdrant_shard_service.retry as retry_module
from services.qdrant_shard_service.metrics import MetricsCollector
from services.qdrant_shard_service.service import ShardClusterService
...
async def test_cache_manager_redis_paths() -> None:
    metrics = MetricsCollector(f"qdrant_shard-redis-{uuid.uuid4().hex[:6]}")
...
async def test_cache_manager_redis_failures() -> None:
    metrics = MetricsCollector(f"qdrant_shard-fail-{uuid.uuid4().hex[:6]}")
    with mock.patch.object(cache_module, "aioredis", _FakeAioredis()):
        cache = cache_module.CacheManager(redis_url="redis://fake", metrics=metrics)
...
async def test_retry_engine_coverage() -> None:
    metrics = MetricsCollector(f"qdrant_shard-retry-{uuid.uuid4().hex[:6]}")
...
async def test_service_edge_cases() -> None:
    metrics = MetricsCollector(f"qdrant_shard-edge-{uuid.uuid4().hex[:6]}")
    service = ShardClusterService(backend="qdrant", redis_url="", metrics=metrics)
