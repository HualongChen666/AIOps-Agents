# -*- coding: utf-8 -*-
"""API tests for the Velero Backup microservice."""
...
from services.velero_backup_service import config
from services.velero_backup_service import main_app as main_module
from services.velero_backup_service.config import settings
from services.velero_backup_service.main_app import app
from services.velero_backup_service.metrics import MetricsCollector
from services.velero_backup_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"velero_backup_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/velero-backup/{method.replace('_', '-')}",
                json=payload,
            )
