# -*- coding: utf-8 -*-
"""Lock and idempotency tests for the ELK Stack microservice."""
...
from services.elk_stack_service.lock import LockManager
...
async def test_idempotency_manager():
    metrics = MetricsCollector(f"elk_stack_idemp_{uuid.uuid4().hex[:6]}")
...
async def test_service_idempotent_request():
    metrics = MetricsCollector(f"elk_stack_idemp_req_{uuid.uuid4().hex[:6]}")
    service = Service(redis_url="", metrics=metrics)
    op = OPERATIONS[0]
