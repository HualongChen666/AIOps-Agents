# -*- coding: utf-8 -*-
"""API tests for the API Standards microservice."""
...
from services.api_standards_service import config
from services.api_standards_service import main_app as main_module
from services.api_standards_service.config import settings
from services.api_standards_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"api_standards_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/api-standards/{method.replace('_', '-')}",
                json=payload,
            )
