# -*- coding: utf-8 -*-
"""API tests for the Distributed Tracing microservice."""
...
from services.distributed_tracing_service import config
from services.distributed_tracing_service import main_app as main_module
from services.distributed_tracing_service.config import settings
from services.distributed_tracing_service.main_app import app
from services.distributed_tracing_service.metrics import MetricsCollector
from services.distributed_tracing_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"distributed_tracing_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/distributed-tracing/{method.replace('_', '-')}",
                json=payload,
            )
