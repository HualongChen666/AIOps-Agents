# -*- coding: utf-8 -*-
"""Coverage tests for the pgBackRest Backup microservice."""
...
import services.pgbackrest_backup_service.cache as cache_module
import services.pgbackrest_backup_service.retry as retry_module
from services.pgbackrest_backup_service.metrics import MetricsCollector
from services.pgbackrest_backup_service.service import Service
...
async def test_cache_manager_redis_paths() -> None:
    ...
    metrics = MetricsCollector(f"pgbackrest_backup-redis-{uuid.uuid4().hex[:6]}")
...
async def test_cache_manager_redis_failures() -> None:
    ...
    metrics = MetricsCollector(f"pgbackrest_backup-fail-{uuid.uuid4().hex[:6]}")
    with mock.patch.object(cache_module, "aioredis", _FakeAioredis()):
        cache = cache_module.CacheManager(redis_url="redis://fake", metrics=metrics)
...
async def test_retry_engine_coverage() -> None:
    ...
    metrics = MetricsCollector(f"pgbackrest_backup-retry-{uuid.uuid4().hex[:6]}")
...
async def test_metrics_collector_all_methods() -> None:
    ...
    metrics = MetricsCollector(f"pgbackrest_backup-metrics-{uuid.uuid4().hex[:6]}")
...
async def test_service_edge_cases() -> None:
    ...
    metrics = MetricsCollector(f"pgbackrest_backup-edge-{uuid.uuid4().hex[:6]}")
