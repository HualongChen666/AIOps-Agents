# -*- coding: utf-8 -*-
"""API tests for the Workflow Engine microservice."""
...
from services.workflow_engine_service import config
from services.workflow_engine_service import main_app as main_module
from services.workflow_engine_service.config import settings
from services.workflow_engine_service.main_app import app
from services.workflow_engine_service.metrics import MetricsCollector
from services.workflow_engine_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"workflow_engine_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/workflow-engine/{method.replace('_', '-')}",
                json=payload,
            )
