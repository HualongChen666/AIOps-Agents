# -*- coding: utf-8 -*-
"""API tests for the Cache Optimization microservice."""
...
from services.cache_optimization_service import config
from services.cache_optimization_service import main_app as main_module
from services.cache_optimization_service.config import settings
from services.cache_optimization_service.main_app import app
from services.cache_optimization_service.metrics import MetricsCollector
from services.cache_optimization_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"cache_optimization_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/cache-optimization/{method.replace('_', '-')}",
                json=payload,
            )
