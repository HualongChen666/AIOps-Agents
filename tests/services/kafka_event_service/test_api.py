# -*- coding: utf-8 -*-
"""API tests for the Kafka Event microservice."""
...
from services.kafka_event_service import config
from services.kafka_event_service import main_app as main_module
from services.kafka_event_service.config import settings
from services.kafka_event_service.main_app import app
from services.kafka_event_service.metrics import MetricsCollector
from services.kafka_event_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"kafka_event_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/kafka-event/{method.replace('_', '-')}",
                json=payload,
            )
