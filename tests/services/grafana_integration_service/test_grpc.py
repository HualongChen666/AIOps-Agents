# -*- coding: utf-8 -*-
"""gRPC client/server tests for the Grafana Integration microservice."""
...
from services.grafana_integration_service.grpc.client import GrafanaIntegrationServiceRPCClient
from services.grafana_integration_service.grpc.server import GrafanaIntegrationServiceRPCServer


@pytest.mark.asyncio
async def test_rpc_server_register_and_call():
    server = GrafanaIntegrationServiceRPCServer()
...
async def test_rpc_client_call():
    client = GrafanaIntegrationServiceRPCClient(base_url="http://test")
