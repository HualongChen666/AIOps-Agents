# -*- coding: utf-8 -*-
"""Lock and idempotency tests for the Grafana Integration microservice."""
...
from services.grafana_integration_service.lock import LockManager
...
async def test_idempotency_manager():
    metrics = MetricsCollector(f"grafana_integration_idemp_{uuid.uuid4().hex[:6]}")
...
async def test_service_idempotent_request():
    metrics = MetricsCollector(f"grafana_integration_idemp_req_{uuid.uuid4().hex[:6]}")
    service = Service(redis_url="", metrics=metrics)
    op = OPERATIONS[0]
