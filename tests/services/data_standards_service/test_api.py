# -*- coding: utf-8 -*-
"""API tests for the Data Standards microservice."""
...
from services.data_standards_service import config
from services.data_standards_service import main_app as main_module
from services.data_standards_service.config import settings
from services.data_standards_service.main_app import app
from services.data_standards_service.metrics import MetricsCollector
from services.data_standards_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"data_standards_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/data-standards/{method.replace('_', '-')}",
                json=payload,
            )
