# -*- coding: utf-8 -*-
"""API tests for the Metrics Monitoring microservice."""
...
from services.metrics_monitoring_service import config
from services.metrics_monitoring_service import main_app as main_module
from services.metrics_monitoring_service.config import settings
from services.metrics_monitoring_service.main_app import app
from services.metrics_monitoring_service.metrics import MetricsCollector
from services.metrics_monitoring_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"metrics_monitoring_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/metrics-monitoring/{method.replace('_', '-')}",
                json=payload,
            )
