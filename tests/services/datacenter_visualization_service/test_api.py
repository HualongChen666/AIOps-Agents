# -*- coding: utf-8 -*-
"""API tests for the Datacenter Visualization microservice."""
...
from services.datacenter_visualization_service import config
from services.datacenter_visualization_service import main_app as main_module
from services.datacenter_visualization_service.config import settings
from services.datacenter_visualization_service.main_app import app
from services.datacenter_visualization_service.metrics import MetricsCollector
from services.datacenter_visualization_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"datacenter_visualization_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/datacenter-visualization/{method.replace('_', '-')}",
                json=payload,
            )
