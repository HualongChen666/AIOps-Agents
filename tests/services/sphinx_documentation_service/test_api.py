# -*- coding: utf-8 -*-
"""API tests for the Sphinx Documentation microservice."""
...
from services.sphinx_documentation_service import config
from services.sphinx_documentation_service import main_app as main_module
from services.sphinx_documentation_service.config import settings
from services.sphinx_documentation_service.main_app import app
from services.sphinx_documentation_service.metrics import MetricsCollector
from services.sphinx_documentation_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"sphinx_documentation_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/sphinx-documentation/{method.replace('_', '-')}",
                json=payload,
            )
