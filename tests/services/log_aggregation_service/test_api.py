# -*- coding: utf-8 -*-
"""API tests for the Log Aggregation microservice."""
...
from services.log_aggregation_service import config
from services.log_aggregation_service import main_app as main_module
from services.log_aggregation_service.config import settings
from services.log_aggregation_service.main_app import app
from services.log_aggregation_service.metrics import MetricsCollector
from services.log_aggregation_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"log_aggregation_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/log-aggregation/{method.replace('_', '-')}",
                json=payload,
            )
