# -*- coding: utf-8 -*-
"""API tests for the Kubernetes Orchestration microservice."""
...
from services.kubernetes_orchestration_service import config
from services.kubernetes_orchestration_service import main_app as main_module
from services.kubernetes_orchestration_service.config import settings
from services.kubernetes_orchestration_service.main_app import app
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"kubernetes_orchestration_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/kubernetes-orchestration/{method.replace('_', '-')}",
                json=payload,
            )
