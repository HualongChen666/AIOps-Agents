# -*- coding: utf-8 -*-
"""API tests for the Prometheus Integration microservice."""
...
from services.prometheus_integration_service import config
from services.prometheus_integration_service import main_app as main_module
from services.prometheus_integration_service.config import settings
from services.prometheus_integration_service.main_app import app
from services.prometheus_integration_service.metrics import MetricsCollector
from services.prometheus_integration_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"prometheus_integration_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/prometheus-integration/{method.replace('_', '-')}",
                json=payload,
            )
