# -*- coding: utf-8 -*-
"""API tests for the Cloud Monitoring microservice."""
...
from services.cloud_monitoring_service import config
from services.cloud_monitoring_service import main_app as main_module
from services.cloud_monitoring_service.config import settings
from services.cloud_monitoring_service.main_app import app
from services.cloud_monitoring_service.metrics import MetricsCollector
from services.cloud_monitoring_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"cloud_monitoring_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/cloud-monitoring/{method.replace('_', '-')}",
                json=payload,
            )
