# -*- coding: utf-8 -*-
"""API tests for the GitHub Repository microservice."""
...
from services.github_repository_service import config
from services.github_repository_service import main_app as main_module
from services.github_repository_service.config import settings
from services.github_repository_service.main_app import app
from services.github_repository_service.metrics import MetricsCollector
from services.github_repository_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"github_repository_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/github-repository/{method.replace('_', '-')}",
                json=payload,
            )
