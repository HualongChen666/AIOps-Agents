# -*- coding: utf-8 -*-
"""补充 PostgreSQL shard cluster 核心分支覆盖率测试。"""
...
import services.postgresql_shard_service.cache as cache_module
import services.postgresql_shard_service.retry as retry_module
...
async def test_cache_manager_redis_paths() -> None:
    metrics = MetricsCollector(f"postgresql_shard-redis-{uuid.uuid4().hex[:6]}")
...
async def test_cache_manager_redis_failures() -> None:
    metrics = MetricsCollector(f"postgresql_shard-fail-{uuid.uuid4().hex[:6]}")
    with mock.patch.object(cache_module, "aioredis", _FakeAioredis()):
        cache = cache_module.CacheManager(redis_url="redis://fake", metrics=metrics)
...
async def test_retry_engine_coverage() -> None:
    metrics = MetricsCollector(f"postgresql_shard-retry-{uuid.uuid4().hex[:6]}")
...
async def test_service_edge_cases() -> None:
    metrics = MetricsCollector(f"postgresql_shard-edge-{uuid.uuid4().hex[:6]}")
    service = ShardClusterService(backend="postgresql", redis_url="", metrics=metrics)
