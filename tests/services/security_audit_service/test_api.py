# -*- coding: utf-8 -*-
"""API tests for the Security Audit microservice."""
...
from services.security_audit_service import config
from services.security_audit_service import main_app as main_module
from services.security_audit_service.config import settings
from services.security_audit_service.main_app import app
from services.security_audit_service.metrics import MetricsCollector
from services.security_audit_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"security_audit_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/security-audit/{method.replace('_', '-')}",
                json=payload,
            )
