# -*- coding: utf-8 -*-
"""API tests for the Performance Monitoring microservice."""
...
from services.performance_monitoring_service import config
from services.performance_monitoring_service import main_app as main_module
from services.performance_monitoring_service.config import settings
from services.performance_monitoring_service.main_app import app
from services.performance_monitoring_service.metrics import MetricsCollector
from services.performance_monitoring_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"performance_monitoring_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/performance-monitoring/{method.replace('_', '-')}",
                json=payload,
            )
