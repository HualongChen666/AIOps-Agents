# -*- coding: utf-8 -*-
"""API tests for the Security Scanning microservice."""
...
from services.security_scanning_service import config
from services.security_scanning_service import main_app as main_module
from services.security_scanning_service.config import settings
from services.security_scanning_service.main_app import app
from services.security_scanning_service.metrics import MetricsCollector
from services.security_scanning_service.service import OPERATIONS, Service
...
async def reset_service():
    ...
    metrics = MetricsCollector(f"security_scanning_api_test_{uuid.uuid4().hex[:6]}")
    ...
    async with httpx.AsyncClient(
        ...
        for method in methods:
            ...
            response = await client.post(
                f"/security-scanning/{method.replace('_', '-')}",
                json=payload,
            )
