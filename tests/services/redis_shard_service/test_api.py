# -*- coding: utf-8 -*-
"""API tests for the Redis shard cluster microservice."""
...
from services.redis_shard_service import main_app as main_module
from services.redis_shard_service.main_app import app
...
async def reset_service():
    from services.redis_shard_service import config

    config.settings.redis_url = ""
    config.settings.database_url = ""
    config.settings.qdrant_url = ""
    metrics = MetricsCollector("redis_shard_api_test")
...
async def test_metrics():
    ...
    assert response.status_code == 200
    assert "redis" in response.text or "shard" in response.text
