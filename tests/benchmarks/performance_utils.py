...
from tests.benchmarks.benchmark_base import MetricSample, PerformanceMetricType, PerformanceResult
    ...
    def _take_sample(self):
        ...
        try:
            ...
            self.measurements.append(
                MeasurementResult(
                    name="cpu_usage", value=cpu_percent, unit="percent", timestamp=timestamp
                )
            )
        ...
        try:
            ...
            self.measurements.append(
                MeasurementResult(
                    ...
                    timestamp=timestamp,
                    metadata={
                        "sent_bytes": current_net.bytes_sent - self._initial_net_io.bytes_sent,
        ...
        try:
            ...
            self.measurements.append(
                MeasurementResult(
                    name="thread_count", value=thread_count, unit="count", timestamp=timestamp
                )
            )
        except Exception as e:
        ...
        try:
            ...
            self.measurements.append(
                MeasurementResult(
                    name="file_descriptor_count", value=fd_count, unit="count", timestamp=timestamp
                )
            )
        except Exception as e:
...
class StatisticalAnalyzer:
    ...
    def calculate_percentiles(
        data: List[float], percentiles: List[int] = [50, 90, 95, 99]
    ) -> Dict[int, float]:
        """
    ...
    @staticmethod
    def detect_outliers(
        data: List[float], method: str = "iqr", multiplier: float = 1.5
    ...
    def calculate_trend(data: List[float], window_size: int = 5) -> str:
        ...
        earlier = (
            data[-2 * window_size : -window_size]
            if len(data) >= 2 * window_size
            else data[:window_size]
        )
    ...
    def compare_datasets(dataset1: List[float], dataset2: List[float]) -> Dict[str, Any]:
        ...
        if result["dataset1"]["mean"] != 0:
            result["mean_difference_percent"] = (
                (result["dataset2"]["mean"] - result["dataset1"]["mean"])
                / result["dataset1"]["mean"]
            ) * 100
        else:
        ...
        if len(dataset1) > 1 and len(dataset2) > 1:
            ...
            pooled_std = (
                (result["dataset1"]["std_dev"] ** 2 / len(dataset1))
                + (result["dataset2"]["std_dev"] ** 2 / len(dataset2))
            ) ** 0.5
            if pooled_std > 0:
                t_stat = (result["dataset2"]["mean"] - result["dataset1"]["mean"]) / pooled_std
                result["t_statistic"] = t_stat
                result["significantly_different"] = (
                    abs(t_stat) > 1.96
                )  # Approximate for 95% confidence
            else:
    ...
    def compare(self, current_result: PerformanceResult) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "duration_change_percent": (
                ((current_result.duration - self.baseline.duration) / self.baseline.duration * 100)
                if self.baseline.duration > 0
                else 0.0
            ),
            "metrics": {},
        }
        ...
        for metric_type in all_types:
            ...
            comparison["metrics"][metric_type.value] = {
                ...
                "regression_detected": stats_comparison.get("significantly_different", False)
                and stats_comparison.get("mean_difference_percent", 0) > 10,
            }
...
class ConcurrencyTester:
    ...
    async def test_async_concurrency(
        self, func: Callable, concurrency: int, total_requests: int
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "statistics": (
                StatisticalAnalyzer.compare_datasets([], response_times) if response_times else {}
            ),
        }
