...
async def test_notification_query_and_read_not_found():
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        result = await notify_engine.get_notification_history(
            limit=5
...
async def test_send_slack_notification_retry(monkeypatch):
    ...
    result = await notify_engine.send_slack_notification(
        "m", "#c", max_retries=2
    )  # noqa: F841  # Variable for test verification
    assert result["success"] is True
...
async def test_send_alert_notification_critical_all_channels(monkeypatch):
    ...
    result = (
        await notify_engine.send_alert_notification(  # noqa: F841  # Variable for test verification
