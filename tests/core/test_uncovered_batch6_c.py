...
def gitops_run(monkeypatch):
    ...
    run_mock = MagicMock(return_value=subprocess.CompletedProcess(args=[], returncode=0, stdout='ok', stderr=''))
...
def test_apply_manifest_failure(gitops_run, tmp_path):
    ...
    gitops_run.return_value = subprocess.CompletedProcess(args=[], returncode=1, stdout='', stderr='fail')
    mgr = gitops.GitOpsManager()
...
def test_get_rollout_status_success(gitops_run):
    gitops_run.return_value = subprocess.CompletedProcess(args=[], returncode=0, stdout='status: healthy\n' * 15, stderr='')
    mgr = gitops.GitOpsManager()
...
def test_get_rollout_status_no_argo(monkeypatch):
    monkeypatch.setattr('core.gitops_manager.shutil.which', lambda name: None if name == 'argo' else '/fake/' + name)
    mgr = gitops.GitOpsManager()
    assert mgr.get_rollout_status('my-rollout') is None
...
def test_get_resource_success(gitops_run):
    gitops_run.return_value = subprocess.CompletedProcess(args=[], returncode=0, stdout='yaml content', stderr='')
    mgr = gitops.GitOpsManager()
...
def test_get_resource_no_kubectl(monkeypatch):
    monkeypatch.setattr('core.gitops_manager.shutil.which', lambda name: None if name == 'kubectl' else '/fake/' + name)
    mgr = gitops.GitOpsManager()
    assert mgr.get_resource('pods', 'web') is None
...
def test_disaster_recover_healthy(gitops_run):
    gitops_run.return_value = subprocess.CompletedProcess(args=[], returncode=0, stdout='status: healthy', stderr='')
    mgr = gitops.GitOpsManager()
...
def test_graphql_host_health_success(monkeypatch):
    ...
    r = asyncio.run(ge.schema.execute('{ hostHealth(hostId: "h1") { hostId status lastChecked details } }'))
...
def test_graphql_incidents_success(monkeypatch):
    ...
    r = asyncio.run(ge.schema.execute('{ incidents(hostId: "h1", limit: 10) { incidentId hostId alertId scriptKey status severity } }'))
    assert r.errors is None
