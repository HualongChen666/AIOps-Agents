...
def test_cli_executor_json_stdout(monkeypatch):
    ...
    cli = CliExecutor(dry_run=False)
    result = cli.run("echo", args=["hello"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_ansible_executor_args_none(monkeypatch, tmp_path):
    ...
    result = ansible.run(args=None)  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_ansible_executor_args_string(monkeypatch, tmp_path):
    ...
    ansible = AnsibleExecutor(dry_run=False)
    result = ansible.run(args="site.yml --check")  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_ansible_executor_empty_args_fallback(monkeypatch, tmp_path):
    ...
    ansible = AnsibleExecutor(dry_run=False)
    result = ansible.run(args=[])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
    assert result["command"] == "ansible-playbook"
...
def test_ansible_executor_relative_playbook_parent_dot(monkeypatch, tmp_path):
    ...
    original_cwd = os.getcwd()  # noqa: F841  # Variable for test verification
    try:
        os.chdir(str(playbooks))
        ansible = AnsibleExecutor(dry_run=False)
        result = ansible.run(args=["site.yml"])  # noqa: F841  # Variable for test verification
        assert result["status"] == "ok"
    finally:
...
def test_ansible_executor_extra_vars_key_value(monkeypatch, tmp_path):
    ...
    ansible = AnsibleExecutor(dry_run=False)
    result = ansible.run(  # noqa: F841  # Variable for test verification
...
def test_k8s_executor_string_input(monkeypatch):
    ...
    result = k8s.run("kubectl get pods")  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_k8s_executor_unsupported_token_defaults_kubectl(monkeypatch):
    ...
    k8s = K8sExecutor(dry_run=False)
    result = k8s.run(["unknown", "get", "pods"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_k8s_executor_istioctl_dry_run():
    ...
    k8s = K8sExecutor(dry_run=True)
    result = k8s.run(["istioctl", "get", "routes"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_k8s_executor_all_supported_tools(monkeypatch):
    ...
    for tool in ["kubectl", "helm", "istioctl", "chaosctl", "velero", "pgbackrest"]:
        result = k8s.run([tool, "version"])  # noqa: F841  # Variable for test verification
        assert result["status"] == "ok"
...
def test_base_infra_service_unmapped_operation():
    ...
    service = TestService()
    result = service.execute_operation("test_op")  # noqa: F841  # Variable for test verification
    assert result["success"] is False
...
def test_base_infra_service_get_state():
    ...
    service = BaseInfraService()
    result = service.execute_operation("get_state")  # noqa: F841  # Variable for test verification
    assert result["success"] is True
...
def test_terraform_executor_real_branches(monkeypatch):
    ...
    result = terraform.run(["plan"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_helm_executor_real_branches(monkeypatch):
    ...
    result = helm.run(["list"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
...
def test_k8s_executor_pgbackrest(monkeypatch):
    ...
    k8s = K8sExecutor(dry_run=False)
    result = k8s.run(["pgbackrest", "info"])  # noqa: F841  # Variable for test verification
    assert result["status"] == "ok"
