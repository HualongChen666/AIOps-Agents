...
def test_correct_experience_exception_handling(client, orchestrator_with_cache):
    ...
    response = client.post(
        "/experience/correct?situation=test&action=test&corrected_by=user"
    )
    assert response.status_code == 500
...
def test_accumulate_knowledge_happy_path(client, orchestrator_with_cache):
    ...
    response = client.post(
        ...
        json={
            "entries": [
                {"subject": "host", "predicate": "has", "object": "disk", "weight": 1.0}
            ]
        },
    )
    assert response.status_code == 200
...
def test_startup_event(client, orchestrator_with_cache):
    ...
    from extensions.addons.operations.scenario_memory_service.main_app import get_orchestrator, startup
    import asyncio
