...
def test_update_dashboard_widget_unavailable(client):
    ...
    with patch("api.frontend_enhancement_router.FRONTEND_AVAILABLE", False):
        resp = client.put(
            "/api/v1/frontend/dashboard/dash-1/widget/widget-1",
            json={"title": "Updated"}
        )
        assert resp.status_code == 503
        # The error message may be in different format due to global error handler
...
def test_update_dashboard_widget_not_found(client):
    ...
    with patch("api.frontend_enhancement_router.frontend_enhancement_manager") as mock_mgr:
        ...
        resp = client.put(
            "/api/v1/frontend/dashboard/dash-1/widget/nonexistent",
            json={"title": "Updated"}
        )
        assert resp.status_code == 404
        # The error message may be in different format due to global error handler
...
def test_update_accessibility_settings_success(client):
    ...
    resp = client.put(
        ...
        json={"high_contrast": True, "font_size": "large"}
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "success"
    ...
    with patch("api.frontend_enhancement_router.FRONTEND_AVAILABLE", False):
        ...
        with TestClient(app) as client:
            ...
            endpoints = [
                ...
                ("/api/v1/frontend/themes/custom", "post", {"theme_id": "t1", "name": "Test", "colors": {}, "base_theme": "light"}),
                ("/api/v1/frontend/dashboard/dash-1", "get", None),
                ("/api/v1/frontend/dashboard/widget", "post", {"dashboard_id": "d1", "widget_id": "w1", "widget_type": "test", "title": "Test", "position": {}}),
                ("/api/v1/frontend/dashboard/dash-1/widget/w-1", "delete", None),
                ("/api/v1/frontend/dashboard/dash-1/widget/w-1", "put", {"title": "Updated"}),
                ("/api/v1/frontend/reports/templates", "post", {"template_id": "t1", "name": "Test", "description": "Test", "data_sources": [], "visualization_config": {}}),
