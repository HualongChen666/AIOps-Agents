...
from api.batch_router import router, batch_get_alerts, batch_get_metrics
...
class TestBatchGetAlerts:
    ...
    @patch('core.alert_engine.alert_history', new=[
        {"id": f"alert-{i}", "title": f"告警{i}", "level": "critical"} 
        for i in range(100)
    ])
    ...
    def test_batch_get_alerts_alert_without_id(self):
        ...
        assert result["results"] == [None, {"id": "alert-2", "title": "内存告警", "level": "warning"}]
