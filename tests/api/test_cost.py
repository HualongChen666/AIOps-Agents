...
@pytest.fixture
def mock_auth():
    """Mock authentication for cost tests"""
    user = Mock()
    user.id = "admin-1"
    user.username = "admin"
    user.role = "admin"
...
def test_cost_endpoint(client, admin_headers, mock_auth, method, path, body, params, expected):
...
def test_cost_collect_with_date_range(client, admin_headers, mock_auth):
...
def test_cost_forecast_with_custom_days(client, admin_headers, mock_auth):
...
def test_cost_budget_details(client, admin_headers, mock_auth):
