...
def sample_optimization_request():
    ...
    return {"resource_id": "opt-1", "action": "apply"}


@pytest.fixture
def sample_report_request():
    """Create a sample report request for testing"""
    return {"period": "30d", "format": "json", "include_forecast": True}
...
class TestCostAnalytics:
    ...
    async def test_get_cost_analytics_exception_handling(self):
        ...
        with patch(
            "api.cost_advanced_router.collect_costs", side_effect=Exception("Analytics error")
        ):
            with pytest.raises(HTTPException) as exc_info:
...
class TestBudgets:
    ...
    async def test_create_budget_validation_error(self):
        ...
        with pytest.raises(Exception):
            BudgetCreate(name="Test", service="EC2", amount=-100.0)  # Should fail gt=0 validation
...
class TestCostAlerts:
    ...
    async def test_create_alert_validation_error(self):
        ...
        with pytest.raises(Exception):
            AlertCreate(
                name="Test", type="budget_exceeded", threshold=-10.0  # Should fail gt=0 validation
            )
...
class TestDataValidation:
    ...
    def test_budget_create_invalid_amount(self):
        ...
        with pytest.raises(Exception):
            BudgetCreate(name="Test", service="EC2", amount=-100.0)

    def test_budget_create_invalid_amount_zero(self):
        """Test that zero amount is rejected"""
        with pytest.raises(Exception):
            BudgetCreate(name="Test", service="EC2", amount=0.0)
    ...
    def test_alert_create_invalid_threshold(self):
        ...
        with pytest.raises(Exception):
            AlertCreate(name="Test", type="budget_exceeded", threshold=-10.0)
...
class TestErrorHandling:
    ...
    async def test_cost_monitor_exception_handling(self):
        ...
        with patch(
            "api.cost_advanced_router.collect_costs", side_effect=Exception("Monitor error")
        ):
            with pytest.raises(HTTPException) as exc_info:
                await get_cost_overview()
    ...
    async def test_forecast_exception_handling(self):
        ...
        with patch(
            "api.cost_advanced_router.forecast_costs", side_effect=Exception("Forecast error")
        ):
            with pytest.raises(HTTPException) as exc_info:
                await get_cost_overview()
    ...
    async def test_budget_status_exception_handling(self):
        ...
        with patch(
            "api.cost_advanced_router.budget_status", side_effect=Exception("Budget status error")
        ):
            with pytest.raises(HTTPException) as exc_info:
                await get_cost_overview()
...
class TestIntegration:
    ...
    async def test_full_alert_lifecycle(self, sample_alert):
        ...
        from api.cost_advanced_router import _alerts, create_alert, get_alerts
    ...
    async def test_optimization_lifecycle(self):
        ...
        from api.cost_advanced_router import get_optimization_suggestions, handle_optimization
    ...
    async def test_report_with_forecast_integration(self):
        ...
        from api.cost_advanced_router import generate_report, get_forecasts
