...
class TestWidgetEndpoints:
    ...
    async def test_create_dashboard_widget_success(self, mock_admin_user, clear_data):
        ...
        result = await router.create_dashboard_widget(widget_create, request, current_user=mock_admin_user)
    ...
    @pytest.mark.asyncio
    async def test_create_dashboard_widget_validation_description_max(self, mock_admin_user, clear_data):
    ...
    @pytest.mark.asyncio
    async def test_create_dashboard_widget_validation_refresh_interval_min(self, mock_admin_user, clear_data):
    ...
    @pytest.mark.asyncio
    async def test_create_dashboard_widget_validation_refresh_interval_max(self, mock_admin_user, clear_data):
        """Test dashboard widget creation with refresh interval above max"""
        with pytest.raises(Exception):  # Pydantic validation error
            DashboardWidgetCreate(
                widget_type=WidgetType.METRIC,
                title="Test",
                refresh_interval=3601
            )
    ...
    @pytest.mark.asyncio
    async def test_update_dashboard_widget_success(self, mock_admin_user, sample_widget, clear_data):
    ...
    @pytest.mark.asyncio
    async def test_delete_dashboard_widget_success(self, mock_admin_user, sample_widget, clear_data):
        ...
        await router.delete_dashboard_widget(sample_widget.id, request, current_user=mock_admin_user)
    ...
    async def test_delete_dashboard_widget_not_found(self, mock_admin_user, clear_data):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await router.delete_dashboard_widget("nonexistent", request, current_user=mock_admin_user)
        
        assert exc_info.value.status_code == status.HTTP_404_NOT_FOUND
        assert exc_info.value.detail == "Widget not found"

    @pytest.mark.asyncio
    async def test_delete_dashboard_widget_cascades_to_layouts(self, mock_admin_user, sample_widget, clear_data):
...
class TestLayoutEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_update_dashboard_layout_success(self, mock_admin_user, sample_layout, clear_data):
    ...
    async def test_delete_dashboard_layout_not_found(self, mock_admin_user, clear_data):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await router.delete_dashboard_layout("nonexistent", request, current_user=mock_admin_user)
...
class TestIntegration:
    ...
    async def test_full_widget_workflow(self, mock_admin_user, clear_data):
        ...
        widget = await router.create_dashboard_widget(widget_create, request, current_user=mock_admin_user)
...
class TestErrorHandling:
    ...
    async def test_concurrent_widget_creation(self, mock_admin_user, clear_data):
        ...
        async def create_widget():
            widget_create = DashboardWidgetCreate(
                widget_type=WidgetType.METRIC,
                title=f"Widget-{asyncio.current_task().get_name()}"
            )
            await router.create_dashboard_widget(widget_create, request, current_user=mock_admin_user)
