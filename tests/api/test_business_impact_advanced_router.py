...
def mock_business_impact_engine():
    ...
    engine = Mock()
    engine.assess_business_impact = AsyncMock(return_value={
        "name": "test-service",
        "impactScore": 7.5,
        "revenueImpact": 10000,
        "affectedUsers": 500,
    ...
    engine.list_business_impact_services = AsyncMock(return_value=[
        {
            "id": "svc-001",
    ...
    engine.list_business_impact_ux_metrics = AsyncMock(return_value=[
        {
...
class TestAnalysisEndpoints:
    ...
    def test_create_analysis_success(self, client, tmp_path, mock_business_impact_engine):
        ...
        with patch('api.business_impact_advanced_router.ANALYSIS_FILE', analysis_file):
            with patch('api.business_impact_advanced_router.DEPENDENCIES_FILE', dependencies_file):
                with patch('api.business_impact_advanced_router.assess_business_impact', mock_business_impact_engine.assess_business_impact):
                    with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
    ...
    def test_create_analysis_engine_failure(self, client, tmp_path):
        ...
        with patch('api.business_impact_advanced_router.ANALYSIS_FILE', analysis_file):
            with patch('api.business_impact_advanced_router.DEPENDENCIES_FILE', dependencies_file):
                with patch('api.business_impact_advanced_router.assess_business_impact', side_effect=Exception("Engine error")):
                    response = client.post(
                        "/api/v1/business-impact/analysis",
                        json={
                            "service_name": "payment-service",
...
class TestMetricsEndpoint:
    ...
    def test_get_business_impact_metrics(self, client, mock_business_impact_engine):
        ...
        with patch('api.business_impact_advanced_router.list_business_impact_services', mock_business_impact_engine.list_business_impact_services):
            with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
    ...
    def test_get_business_impact_metrics_with_service_filter(self, client, mock_business_impact_engine):
        """Test getting metrics for specific service"""
        with patch('api.business_impact_advanced_router.list_business_impact_services', mock_business_impact_engine.list_business_impact_services):
            with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
                response = client.get("/api/v1/business-impact/metrics?service_name=payment-service")
                assert response.status_code == 200
                data = response.json()
                assert data["success"] is True
    ...
    def test_get_business_impact_metrics_service_not_found(self, client, mock_business_impact_engine):
        """Test getting metrics for non-existent service"""
        with patch('api.business_impact_advanced_router.list_business_impact_services', mock_business_impact_engine.list_business_impact_services):
            with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
                response = client.get("/api/v1/business-impact/metrics?service_name=non-existent-service")
    ...
    def test_get_business_impact_metrics_engine_failure(self, client):
        ...
        with patch('api.business_impact_advanced_router.list_business_impact_services', side_effect=Exception("Engine error")):
            response = client.get("/api/v1/business-impact/metrics")
            # API returns 200 even for error responses
            assert response.status_code == 200
            data = response.json()
...
class TestDependenciesEndpoints:
    ...
    def test_get_dependencies_empty(self, client, tmp_path):
        ...
        with patch('api.business_impact_advanced_router.DEPENDENCIES_FILE', tmp_path / "dependencies.json"):
    ...
    def test_get_dependencies_with_target_filter(self, client, tmp_path, sample_dependency):
        ...
        with patch('api.business_impact_advanced_router.DEPENDENCIES_FILE', dependencies_file):
            response = client.get("/api/v1/business-impact/dependencies?target_service=database-service")
            assert response.status_code == 200
            data = response.json()
            assert data["success"] is True
            assert len(data["data"]["items"]) == 1
...
class TestReportsEndpoints:
    ...
    def test_create_report_without_recommendations(self, client, tmp_path, mock_business_impact_engine):
    ...
    def test_create_report_with_failed_service(self, client, tmp_path, mock_business_impact_engine):
        ...
        mock_business_impact_engine.assess_business_impact = AsyncMock(side_effect=[
...
class TestImpactScoresEndpoint:
    ...
    def test_get_impact_scores_with_service_filter(self, client, mock_business_impact_engine):
        ...
        with patch('api.business_impact_advanced_router.list_business_impact_services', mock_business_impact_engine.list_business_impact_services):
            response = client.get("/api/v1/business-impact/impact-scores?service_name=payment-service")
            assert response.status_code == 200
            data = response.json()
            assert data["success"] is True
...
class TestErrorHandling:
    ...
    def test_exception_handling_in_get_analysis_list(self, client, tmp_path):
        ...
        with patch('api.business_impact_advanced_router.ANALYSIS_FILE', tmp_path / "analysis.json"):
            with patch('api.business_impact_advanced_router._load_json_file', side_effect=Exception("Test error")):
                response = client.get("/api/v1/business-impact/analysis")
                assert response.status_code == 200
                data = response.json()
                assert data["success"] is False
    ...
    def test_exception_handling_in_create_analysis(self, client, tmp_path):
        ...
        analysis_file = tmp_path / "analysis.json"
        with patch('api.business_impact_advanced_router.ANALYSIS_FILE', analysis_file):
            with patch('api.business_impact_advanced_router._save_json_file', side_effect=Exception("Save error")):
                response = client.post(
                    "/api/v1/business-impact/analysis",
            ...
            with patch('api.business_impact_advanced_router.DEPENDENCIES_FILE', dependencies_file):
                with patch('api.business_impact_advanced_router.assess_business_impact', mock_business_impact_engine.assess_business_impact):
                    with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
                        ...
                        # Filter by service
                        filter_response = client.get("/api/v1/business-impact/analysis?service_name=payment-service")
    ...
    def test_metrics_and_impact_scores_workflow(self, client, mock_business_impact_engine):
        ...
        with patch('api.business_impact_advanced_router.list_business_impact_services', mock_business_impact_engine.list_business_impact_services):
            with patch('api.business_impact_advanced_router.list_business_impact_ux_metrics', mock_business_impact_engine.list_business_impact_ux_metrics):
                ...
                assert metrics_data["summary"]["total_services"] == scores_data["statistics"]["count"]
