...
class MockDocumentationManager:
    ...
    def list_documents(self, doc_type=None, status=None):
        ...
        return [{"doc_id": d.doc_id, "title": d.title, "doc_type": d.doc_type.value if hasattr(d.doc_type, 'value') else str(d.doc_type),
                 "status": d.status.value if hasattr(d.status, 'value') else str(d.status), "version": d.version, "author": d.author} for d in docs]
...
def client(mock_manager, mock_doc_type, mock_doc_status):
    ...
    with patch('api.documentation_advanced_router.DOCUMENTATION_AVAILABLE', True):
        with patch('api.documentation_advanced_router.get_documentation_manager', return_value=fresh_manager):
...
class TestListDocuments:
    ...
    def test_list_documents_with_filters(self, client, mock_doc_type):
        ...
        response = client.get("/api/v1/documentation/documents?doc_type=api_documentation&status=draft")
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
...
class TestUpdateDocument:
    ...
    def test_update_document_metadata(self, client, mock_doc_type):
        ...
        response = client.patch(
            f"/api/v1/documentation/documents/{doc_id}",
            json={
                "metadata": {"category": "technical", "tags": ["api", "rest"]}
            }
        )
        assert response.status_code == 200
    ...
    def test_update_document_not_found(self, client):
        ...
        response = client.patch(
            "/api/v1/documentation/documents/nonexistent",
            json={
                "content": "New Content"
            }
        )
    ...
    def test_update_document_service_unavailable(self):
        ...
        with patch('api.documentation_advanced_router.DOCUMENTATION_AVAILABLE', False):
            ...
            response = client.patch(
                "/api/v1/documentation/documents/doc-001",
...
class TestDocumentVersions:
    ...
    def test_list_document_versions_success(self, client, mock_doc_type):
        ...
        document_versions[doc_id] = [
            {"version": "1.0", "content": "Content v1", "created_at": "2024-01-01", "author": "John"},
            {"version": "1.1", "content": "Content v2", "created_at": "2024-01-02", "author": "John"}
        ]
