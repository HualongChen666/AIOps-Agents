...
class TestIntegration:
    ...
    def test_full_notification_lifecycle(self, client, sample_channel, sample_template):
        """Test full notification lifecycle: create channel, template, rule, send notification"""
        # Create channel
        channel_data = {
            "name": "Test Channel",
            "type": "email",
        ...
        channel = client.get(f"/api/v1/notify/channels/{channel_id}")
        assert channel.status_code == 200

        template = client.get(f"/api/v1/notify/templates/{template_id}")
        assert template.status_code == 200

        rule = client.get(f"/api/v1/notify/rules/{rule_id}")
        assert rule.status_code == 200

        # Cleanup
        client.delete(f"/api/v1/notify/rules/{rule_id}")
        client.delete(f"/api/v1/notify/templates/{template_id}")
        client.delete(f"/api/v1/notify/channels/{channel_id}")
