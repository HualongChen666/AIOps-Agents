...
class TestPluginInstallUninstall:
    ...
    def test_install_plugin_success(self, client, sample_listing):
        ...
        install_data = {"version": "1.0.0", "config": {"test": True}}
    ...
    def test_install_plugin_not_found(self, client):
        ...
        response = client.post(
            f"/api/v1/plugin/marketplace/plugins/{fake_id}/install", json=install_data
        )
        assert response.status_code == 404
...
class TestReviewEndpoints:
    ...
    def test_get_plugin_reviews_empty(self, client, sample_listing):
        ...
        response = client.get(
            f"/api/v1/plugin/marketplace/plugins/{sample_listing['plugin_id']}/reviews"
        )
        assert response.status_code == 200
        assert response.json() == []
...
class TestErrorHandling:
    ...
    def test_get_plugins_exception_handling(self, mock_logger, client):
        ...
        with patch(
            "api.plugin_marketplace_advanced_router.list", side_effect=Exception("Test error")
        ):
            response = client.get("/api/v1/plugin/marketplace/plugins")
    ...
    def test_create_review_exception_handling(self, mock_logger, client, sample_listing):
        ...
        with patch(
            "api.plugin_marketplace_advanced_router.uuid4", side_effect=Exception("Test error")
        ):
            review_data = {
                "plugin_id": sample_listing["plugin_id"],
                "reviewer": "Test",
    ...
    def test_install_plugin_exception_handling(self, mock_logger, client, sample_listing):
        ...
        with patch(
            "api.plugin_marketplace_advanced_router.InstallResponse",
            side_effect=Exception("Test error"),
        ):
            install_data = {"version": "1.0.0"}
            response = client.post(
                f"/api/v1/plugin/marketplace/plugins/{sample_listing['id']}/install",
                json=install_data,
            )
            assert response.status_code == 500
...
class TestDataValidation:
    ...
    def test_install_request_invalid_version_format(self, client, sample_listing):
        ...
        install_data = {"version": "invalid.version", "config": {}}
...
class TestIntegration:
    ...
    def test_category_and_plugin_filtering(self, client, sample_category, sample_listing):
        ...
        plugins_response = client.get(
            f"/api/v1/plugin/marketplace/plugins?category={sample_category['name']}"
        )
...
if __name__ == "__main__":
    pytest.main(
        [__file__, "-v", "--cov=api.plugin_marketplace_advanced_router", "--cov-report=html"]
    )
