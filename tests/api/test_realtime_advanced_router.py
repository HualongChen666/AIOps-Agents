...
class TestCreateRealtimeStream:
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_stream_success(
        self, mock_get_db, client, sample_stream_create, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_stream_invalid_stream_type(
        self, mock_get_db, client, sample_stream_create
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_stream_duplicate_name(
        self, mock_get_db, client, sample_stream_create, mock_realtime_stream
    ):
...
class TestUpdateRealtimeStream:
    ...
    def test_update_realtime_stream_success(
        self, mock_get_db, client, sample_stream_update, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_update_realtime_stream_invalid_stream_type(
        self, mock_get_db, client, mock_realtime_stream
    ):
...
class TestGetRealtimeSubscriptions:
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_get_realtime_subscriptions_success(
        self, mock_get_db, client, mock_realtime_subscription
    ):
        """Test successful retrieval of subscriptions"""
        mock_db = Mock()
        mock_get_db.return_value = mock_db
        mock_db.query.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            mock_realtime_subscription
        ]
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_get_realtime_subscriptions_with_filters(
        self, mock_get_db, client, mock_realtime_subscription
    ):
        ...
        response = client.get(
            "/api/v1/realtime/subscriptions?stream_id=STR-001&subscriber_id=user-001&status=active"
        )
...
class TestCreateRealtimeSubscription:
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_subscription_success(
        self, mock_get_db, client, sample_subscription_create, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_subscription_stream_not_found(
        self, mock_get_db, client, sample_subscription_create
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_subscription_invalid_subscription_type(
        self, mock_get_db, client, sample_subscription_create, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_subscription_db_error(
        self, mock_get_db, client, sample_subscription_create
    ):
...
class TestCreateRealtimeWebhook:
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_webhook_success(
        self, mock_get_db, client, sample_webhook_create, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_webhook_stream_not_found(
        self, mock_get_db, client, sample_webhook_create
    ):
        ...
        response = client.post("/api/v1/realtime/webhooks", json=sample_webhook_create.model_dump())

        assert response.status_code == 404
        assert "不存在" in response.json()["detail"]

    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_webhook_invalid_http_method(
        self, mock_get_db, client, sample_webhook_create, mock_realtime_stream
    ):
    ...
    @patch("api.realtime_advanced_router.get_db")
    def test_create_realtime_webhook_duplicate_name(
        self,
        mock_get_db,
        client,
...
class TestDataValidation:
    ...
    def test_stream_create_invalid_stream_type(self):
        ...
        with pytest.raises(Exception):
            RealtimeStreamCreate(name="测试流", stream_type="invalid_type", config={})
    ...
    def test_subscription_create_invalid_subscription_type(self):
        ...
        with pytest.raises(Exception):
            RealtimeSubscriptionCreate(
                stream_id="STR-001", subscriber_id="user-001", subscription_type="invalid_type"
            )
    ...
    def test_webhook_create_invalid_url(self):
        ...
        with pytest.raises(Exception):
            RealtimeWebhookCreate(name="测试Webhook", url="invalid_url", method="POST")
...
class TestIntegration:
    ...
    def test_full_stream_lifecycle(
        self, mock_get_db, client, sample_stream_create, sample_stream_update, mock_realtime_stream
    ):
