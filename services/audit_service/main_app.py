...
from services.audit_service.repository import get_repository
...
async def get_orchestrator() -> AuditOrchestrator:
    """Return the shared orchestrator, backed by the configured repository."""
    global _orchestrator
    if _orchestrator is None:
        repository = await get_repository(settings.use_in_memory)
        _orchestrator = AuditOrchestrator(repository)
