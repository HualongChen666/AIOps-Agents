...
**最后更新**: 2026-08-25  
**GitHub Dependabot状态**: 仍然显示71个漏洞（等待重新扫描）  
**本地依赖冲突**: 已解决（protobuf版本冲突）
...
### GitHub Dependabot重新扫描延迟

GitHub Dependabot需要时间来重新扫描更新后的依赖项。当前仍然显示71个漏洞是因为：

1. Dependabot使用缓存数据，需要时间更新
2. 通常需要几小时到1天来重新扫描
3. 建议等待24小时后再次检查
...
- **protobuf版本冲突**: 已将protobuf从>=5.29.6改为>=3.19,<5.0以兼容OpenTelemetry
- **OpenTelemetry版本一致性**: 已将所有OpenTelemetry包更新到1.27.0/0.48b0版本集
