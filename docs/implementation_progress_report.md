...
### 高优先级（关键功能）
1. **阶段3：迁移ai_advanced_router.py** - AI功能相关，可能比alerts更简单
2. **阶段5：迁移其他内存存储router** - 成本、容量、变更、资产等
3. **阶段2：迁移alerts_advanced_router.py** - 需要更详细的分阶段迁移策略

### 中优先级（增强功能）
4. **阶段8：数据迁移与验证** - 需要先完成数据库迁移
5. **阶段10：测试验证** - 需要先完成API实现
