...
> ⚠️ **层定义权威说明**：本文件下方使用的层名（API网关 / 业务逻辑 / AI引擎 / 数据访问 /
> 数据存储 / 集成 / 监控）描述的是**目标态部署拓扑**。当前代码中已实现的 7 层定义以
> `README.md` 的「7-Layer Platform Architecture」为准——L1 实时流处理 / L2 分析 /
> L3 处理 / L4 存储 / L5 知识 / L6 执行 / L7 集成，对应 `core/l1l2_data_flow_integrator.py`
> 等六个真实集成器。两套命名不得混用。
