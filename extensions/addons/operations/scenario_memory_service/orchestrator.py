...
# Tokenization helpers for a simple semantic vectorizer.
_CJK_RE = re.compile(r"[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]")
_ALPHANUM_RE = re.compile(r"[A-Za-z0-9_]+")

# Default TTLs for experience and knowledge entries.
_EXPERIENCE_DEFAULT_TTL_DAYS = 180
_KNOWLEDGE_DEFAULT_TTL_DAYS = 365


def _tokenize(text: str) -> List[str]:
    """Extract CJK bigrams and lowercase alphanumeric tokens from text."""
    tokens: List[str] = []
    text = str(text)
    cjk_chars = _CJK_RE.findall(text)
    for i in range(max(0, len(cjk_chars) - 1)):
        tokens.append(cjk_chars[i] + cjk_chars[i + 1])
    for word in _ALPHANUM_RE.findall(text):
        tokens.append(word.lower())
    return tokens


def _text_to_vector(text: str, dimension: int = 128) -> List[float]:
    """Generate a deterministic normalized vector from text using token n-grams.

    Two texts that share tokens (words or CJK bigrams) will have a higher cosine
    similarity than unrelated texts, unlike the previous SHA256-based approach.
    """
    vector = [0.0] * dimension
    for token in _tokenize(text):
        idx = int(hashlib.sha256(token.encode("utf-8")).hexdigest(), 16) % dimension
        vector[idx] += 1.0
...
class ScenarioMemoryOrchestrator:
    ...
    async def search_similar(self, request: SimilarityQueryRequest) -> SimilarityQueryResponse:
        ...
        for event_id, event in self._events.items():
            if session_id and getattr(event, "session_id", None) != session_id:
                continue
            if event.vector:
    ...
    async def learn_experience(self, request: LearnExperienceRequest) -> LearnExperienceResponse:
        ...
        if ttl_seconds is None or ttl_seconds <= 0:
            ttl_seconds = _EXPERIENCE_DEFAULT_TTL_DAYS * 86400
        expires_at = now + timedelta(seconds=ttl_seconds)

        existing = self._experiences.get(key)
        expired = False
        if existing:
            expired = bool(existing.expires_at and existing.expires_at < now)
            if not existing.valid or expired:
                # Treat a corrected/invalid or expired experience as a new learning.
                existing.valid = True
                existing.corrected_by = None
    ...
    async def accumulate_knowledge(
        ...
        now = datetime.utcnow()
        default_expires = now + timedelta(days=_KNOWLEDGE_DEFAULT_TTL_DAYS)
        ...
        for entry in request.entries:
            ...
            if existing:
                ...
                if not existing.expires_at:
                    existing.expires_at = default_expires
                updated += 1
                ...
                if not entry.expires_at:
                    entry.expires_at = default_expires
                entry.valid = True
    ...
    # ------------------------------------------------------------------
    def _session_key(self, session_id: Optional[str], key: str) -> str:
        """Namespace a key by session id for multi-session isolation."""
        if session_id:
    ...
    async def store_short_term(self, request: ShortTermRequest) -> ShortTermResponse:
        ...
        storage_key = self._session_key(request.session_id, request.key)
        self._short_term[storage_key] = ShortTermMemory(
            key=request.key,
            value=request.value,
            ttl_seconds=request.ttl_seconds,
            session_id=request.session_id,
        )
        await self.cache.set(f"stm:{storage_key}", request.value, ttl=request.ttl_seconds)
    ...
    async def retrieve_short_term(
        ...
        storage_key = self._session_key(session_id, key)
        cached = await self.cache.get(f"stm:{storage_key}")
        if cached is not None:
            return cached
        entry = self._short_term.get(storage_key)
        if entry:
            age = (datetime.utcnow() - entry.timestamp).total_seconds()
            if age < entry.ttl_seconds:
                return entry.value
            self._short_term.pop(storage_key, None)
        return None
    ...
    async def store_long_term(self, request: LongTermRequest) -> LongTermResponse:
        ...
        storage_key = self._session_key(request.session_id, request.key)
        self._long_term[storage_key] = LongTermMemory(
            key=request.key,
            value=request.value,
        ...
        await self.cache.set(
            f"ltm:{storage_key}", request.value, ttl=self.settings.default_cache_ttl * 10
        )
    ...
    async def retrieve_long_term(self, key: str, session_id: Optional[str] = None) -> Optional[Any]:
        self._increment_count("retrieve_long_term")
        storage_key = self._session_key(session_id, key)
        cached = await self.cache.get(f"ltm:{storage_key}")
        if cached is not None:
            return cached
        entry = self._long_term.get(storage_key)
        if entry:
    ...
    # ------------------------------------------------------------------
    # 34.11 Experience expiration / correction
    # ------------------------------------------------------------------
    def _is_valid_experience(self, exp: Experience) -> bool:
        """Return True if an experience is valid and not expired."""
        if not exp.valid:
            return False
        if exp.expires_at and exp.expires_at < datetime.utcnow():
            return False
        return True
    ...
    async def correct_experience(
        ...
        """Mark an existing experience as invalid and optionally create a corrected copy."""
        self._increment_count("correct_experience")
        key = f"{situation.strip()}::{action.strip()}"
        existing = self._experiences.get(key)
        if not existing:
            return None
        ...
        if corrected_outcome:
            new_key = f"{key}:corrected:{self._new_id()}"
            ...
            self._experiences[new_key] = corrected
            corrected_id = corrected.experience_id
        logger.info(f"Experience {key} corrected by {corrected_by}")
        return corrected_id or existing.experience_id
    ...
    async def find_experiences(
        ...
        """Find valid, unexpired experiences matching a query."""
        self._increment_count("find_experiences")
        query_lower = query.lower()
        matches: List[Tuple[float, Experience]] = []
        for exp in self._experiences.values():
            if not self._is_valid_experience(exp):
                continue
            if session_id and exp.session_id != session_id:
                continue
            score = 0.0
            if query_lower in exp.situation.lower():
                score += 1.0
        ...
        return [exp for _, exp in matches[:top_k]]
