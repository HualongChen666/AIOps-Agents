...
async def retrieve_short_term(key: str, session_id: Optional[str] = None) -> Any:
    try:
        value = await get_orchestrator().retrieve_short_term(key, session_id=session_id)
        if value is None:
...
async def retrieve_long_term(key: str, session_id: Optional[str] = None) -> Any:
    try:
        value = await get_orchestrator().retrieve_long_term(key, session_id=session_id)
        if value is None:
...
@app.get("/experiences/find")
async def find_experiences(query: str, top_k: int = 5, session_id: Optional[str] = None) -> dict:
    try:
        experiences = await get_orchestrator().find_experiences(
...
async def correct_experience(
    ...
    try:
        corrected_id = await get_orchestrator().correct_experience(
            situation=situation,
            action=action,
        ...
        return {"corrected_experience_id": corrected_id}
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
