# -*- coding: utf-8 -*-
"""Real business logic for the datadog_integration_service add-on microservice."""
...
SERVICE_NAME = "datadog_integration_service"
...
store: Dict[str, MetricPayload] = {}


def _create(payload: Dict[str, Any]) -> Dict[str, Any]:
    item = MetricPayload(**payload)
    store[item.id] = item
    logger.info("[%s] created %s id=%s", SERVICE_NAME, "metricpayload", item.id)
...
def _get(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not item_id or item_id not in store:
        raise HTTPException(status_code=404, detail="metricpayload not found")
...
def _update(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    updated = MetricPayload(**existing)
    store[item_id] = updated
    logger.info("[%s] updated %s id=%s", SERVICE_NAME, "metricpayload", item_id)
...
def _delete(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not item_id or item_id not in store:
        raise HTTPException(status_code=404, detail="metricpayload not found")
    del store[item_id]
    logger.info("[%s] deleted %s id=%s", SERVICE_NAME, "metricpayload", item_id)
...
def _run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item_id and item_id in store:
        logger.info("[%s] ran %s id=%s", SERVICE_NAME, "metricpayload", item_id)
...
def _import(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item_data in items:
        item = MetricPayload(**item_data)
        store[item.id] = item
        imported += 1
    logger.info("[%s] imported %s %s items", SERVICE_NAME, imported, "metricpayload")
...
async def health() -> HealthResponse:
    return HealthResponse(status="ok", service=SERVICE_NAME, metricpayload_count=len(store))
...
@app.get("/metric_payloads", response_model=List[Dict[str, Any]])
async def list_metric_payloads():
    return _list({})


@app.get("/metric_payloads/{item_id}", response_model=Dict[str, Any])
async def get_metricpayload(item_id: str):
