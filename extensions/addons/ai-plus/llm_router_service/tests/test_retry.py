...
class TestLLMRetryEngine:
    ...
    async def test_execute_max_retries_exceeded(self):
        ...
        # But with non-retryable error, it may only be called once
    ...
    async def test_execute_delay_calculation(self):
        ...
        async def delayed_fn():
            ...
            if len(call_times) < 2:
                raise Exception("retryable error")  # Make it retryable
            return "success"

        # Use a policy that allows retries
        policy = RetryPolicy(name="test", retryable_errors=["retryable"])
        await engine.execute(delayed_fn, policy_name="test")
    ...
    async def test_is_retryable_with_matching_error(self):
        ...
        assert engine._is_retryable(Exception("rate_limit error"), policy) is True
    ...
    async def test_compute_delay_jitter(self):
        ...
        # Jitter should produce different delays (most of the time)
        # But within reasonable bounds (0.5 to 1.5 due to random factor)
