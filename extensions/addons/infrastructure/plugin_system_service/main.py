# -*- coding: utf-8 -*-
"""Real business logic for the plugin_system_service add-on microservice."""
...
SERVICE_NAME = "plugin_system_service"
...
store: Dict[str, PluginInstance] = {}


def _create(payload: Dict[str, Any]) -> Dict[str, Any]:
    item = PluginInstance(**payload)
    store[item.id] = item
    logger.info("[%s] created %s id=%s", SERVICE_NAME, "plugininstance", item.id)
...
def _get(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not item_id or item_id not in store:
        raise HTTPException(status_code=404, detail="plugininstance not found")
...
def _update(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    updated = PluginInstance(**existing)
    store[item_id] = updated
    logger.info("[%s] updated %s id=%s", SERVICE_NAME, "plugininstance", item_id)
...
def _delete(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not item_id or item_id not in store:
        raise HTTPException(status_code=404, detail="plugininstance not found")
    del store[item_id]
    logger.info("[%s] deleted %s id=%s", SERVICE_NAME, "plugininstance", item_id)
...
def _run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item_id and item_id in store:
        logger.info("[%s] ran %s id=%s", SERVICE_NAME, "plugininstance", item_id)
...
def _import(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item_data in items:
        item = PluginInstance(**item_data)
        store[item.id] = item
        imported += 1
    logger.info("[%s] imported %s %s items", SERVICE_NAME, imported, "plugininstance")
...
async def health() -> HealthResponse:
    return HealthResponse(status="ok", service=SERVICE_NAME, plugininstance_count=len(store))
...
@app.get("/plugin_instances", response_model=List[Dict[str, Any]])
async def list_plugin_instances():
    return _list({})


@app.get("/plugin_instances/{item_id}", response_model=Dict[str, Any])
async def get_plugininstance(item_id: str):
