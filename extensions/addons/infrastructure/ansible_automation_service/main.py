# -*- coding: utf-8 -*-
"""Real business logic for the ansible_automation_service add-on microservice."""
...
SERVICE_NAME = "ansible_automation_service"
...
store: Dict[str, Playbook] = {}


def _create(payload: Dict[str, Any]) -> Dict[str, Any]:
    item = Playbook(**payload)
    store[item.id] = item
    logger.info("[%s] created %s id=%s", SERVICE_NAME, "playbook", item.id)
...
def _update(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    updated = Playbook(**existing)
    store[item_id] = updated
    logger.info("[%s] updated %s id=%s", SERVICE_NAME, "playbook", item_id)
...
def _delete(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not item_id or item_id not in store:
        raise HTTPException(status_code=404, detail="playbook not found")
    del store[item_id]
    logger.info("[%s] deleted %s id=%s", SERVICE_NAME, "playbook", item_id)
...
def _run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item_id and item_id in store:
        logger.info("[%s] ran %s id=%s", SERVICE_NAME, "playbook", item_id)
...
def _import(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item_data in items:
        item = Playbook(**item_data)
        store[item.id] = item
        imported += 1
    logger.info("[%s] imported %s %s items", SERVICE_NAME, imported, "playbook")
...
async def health() -> HealthResponse:
    return HealthResponse(status="ok", service=SERVICE_NAME, playbook_count=len(store))
...
@app.get("/playbooks/{item_id}", response_model=Dict[str, Any])
async def get_playbook(item_id: str):
