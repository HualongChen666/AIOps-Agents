...
class TestMainApp:
    ...
    def test_startup_event(self, mock_get_orch):
        """Test startup event connects cache and store (covers lines 53-55)."""
        mock_orch = MagicMock()
        mock_orch.cache = AsyncMock()
        mock_orch.cache.connect = AsyncMock()
        mock_orch.store = AsyncMock()
        mock_orch.store.connect = AsyncMock()
        ...
        mock_orch.cache.connect.assert_called_once()
        mock_orch.store.connect.assert_called_once()
    ...
    def test_build_graph_endpoint(self, mock_get_orch, client):
        ...
        from extensions.addons.ai_plus.knowledge_graph_service.schemas import GraphBuildResponse

        mock_orch = MagicMock()
        mock_response = GraphBuildResponse(
            graph_id="test_graph",
            nodes_count=2,
            edges_count=1,
            built=True,
        )
        mock_orch.build_graph = AsyncMock(return_value=mock_response)
        mock_get_orch.return_value = mock_orch
        ...
        response = client.post("/graph/build", json=request.model_dump(mode="json"))
        assert response.status_code == 200
        data = response.json()
    ...
    @patch("extensions.addons.ai_plus.knowledge_graph_service.main_app.get_orchestrator")
    def test_rpc_with_basemodel_response(self, mock_get_orch, client):
        """Test RPC with BaseModel response (covers line 194)."""
        from extensions.addons.ai_plus.knowledge_graph_service.schemas import EntityModelingResponse

        mock_orch = MagicMock()
        mock_response = EntityModelingResponse(
            node_id="test_node",
            entity_name="Test",
            entity_type="generic",
            modeled=True,
        )
        mock_orch.model_entity = AsyncMock(return_value=mock_response)
        mock_orch.list_methods = MagicMock(return_value=["model_entity"])
        mock_get_orch.return_value = mock_orch
        ...
        assert response.status_code == 200
        data = response.json()
        # Should be converted to dict by model_dump()
        assert isinstance(data, dict)
        assert data["node_id"] == "test_node"

    @patch("extensions.addons.ai_plus.knowledge_graph_service.main_app.get_orchestrator")
    def test_rpc_http_exception_reraise(self, mock_get_orch, client):
        """Test RPC with HTTPException is re-raised (covers line 196)."""
        from fastapi import HTTPException

        mock_orch = MagicMock()
        mock_orch.model_entity = AsyncMock(side_effect=HTTPException(status_code=400, detail="Bad request"))
        mock_orch.list_methods = MagicMock(return_value=["model_entity"])
        mock_get_orch.return_value = mock_orch
