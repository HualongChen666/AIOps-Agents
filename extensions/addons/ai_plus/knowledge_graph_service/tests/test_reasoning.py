...
class TestGraphReasoningEngine:
    ...
    def test_reason_pagerank_single_node(self, reasoning_engine):
        ...
        assert len(response.results) == 1
        # PageRank for single node with damping 0.85: (1-0.85)/1 = 0.15
    ...
    def test_infer_neighbors_with_relation_filter(self, reasoning_engine):
        """Test _infer_neighbors with relation filter (covers line 64)."""
        nodes = [
            GraphNode(node_id="node1", label="Node 1", node_type="entity"),
            GraphNode(node_id="node2", label="Node 2", node_type="entity"),
            GraphNode(node_id="node3", label="Node 3", node_type="entity"),
        ...
        edges = [
            ...
            GraphEdge(
                edge_id="edge2", source_id="node1", target_id="node3", relation="DEPENDS_ON"
            ),
        ]

        # Test with relation filter
        results = reasoning_engine._infer_neighbors("node1", edges, "CONNECTS_TO")
        assert len(results) == 1
        assert results[0]["node_id"] == "node2"
        assert results[0]["relation"] == "CONNECTS_TO"

        # Test without relation filter
        results = reasoning_engine._infer_neighbors("node1", edges, None)
        assert len(results) == 2

    def test_pagerank_empty_graph(self, reasoning_engine):
        """Test PageRank with empty graph (covers line 111)."""
        nodes = []
        edges = []

        results = reasoning_engine._page_rank(nodes, edges)
    ...
    def test_pagerank_iterations(self, reasoning_engine):
        ...
        # Scores should be positive and reasonable
        for result in response.results:
