...
# flake8: noqa
# isort: skip_file
"""Comprehensive 13-dimension verification for tasks 79-98 with per-service coverage."""
...
JSON_OUT = OUT_DIR / "tasks_79_98_final_verification.json"
MD_OUT = OUT_DIR / "tasks_79_98_final_report.md"
...
TASKS = [
    ("79", "sphinx_documentation_service", "基于开源生态的文档体系建设（任务79）"),
    ("80", "security_audit_service", "基于开源工具的安全审计（任务80）"),
    ("81", "backup_recovery_drill_service", "备份恢复演练（任务81）"),
    ("82", "incident_response_service", "应急响应演练（任务82）"),
    ("83", "github_repository_service", "开源仓库建设（任务83）"),
    ("84", "open_source_license_service", "基于开源生态的许可证管理（任务84）"),
    ("85", "plugin_system_service", "基于开源生态的插件架构（任务85）"),
    ("86", "plugin_market_service", "插件市场（任务86）"),
    ("87", "api_standards_service", "基于开源标准的技术规范 - API规范（任务87）"),
    ("88", "data_standards_service", "基于开源标准的技术规范 - 数据规范（任务88）"),
    ("89", "performance_monitoring_service", "基于开源工具的性能优化 - 性能监控（任务89）"),
    ("90", "cache_optimization_service", "基于开源工具的性能优化 - 缓存优化（任务90）"),
    ("91", "database_optimization_service", "基于开源工具的性能优化 - 数据库优化（任务91）"),
    ("92", "security_scanning_service", "基于开源工具的安全加固 - 安全扫描（任务92）"),
    ("93", "penetration_testing_service", "基于开源工具的安全加固 - 渗透测试（任务93）"),
    ("94", "automated_deployment_service", "基于开源工具的运维自动化 - 自动化部署（任务94）"),
    ("95", "automated_ops_service", "基于开源工具的运维自动化 - 自动化运维（任务95）"),
    ("96", "log_aggregation_service", "基于开源工具的可观测性 - 日志聚合（任务96）"),
    ("97", "metrics_monitoring_service", "基于开源工具的可观测性 - 指标监控（任务97）"),
    ("98", "distributed_tracing_service", "分布式追踪（任务98）"),
]
...
def parse_pytest_summary(stdout: str) -> dict:
    out = {}
    m = re.search(
        r"(\d+)\s+passed(?:,\s+(\d+)\s+failed)?(?:,\s+(\d+)\s+skipped)?(?:,\s+(\d+)\s+xfailed)?(?:,\s+\d+\s+warnings?)?\s+in\s+([\d.]+)s",
        stdout,
        re.IGNORECASE,
    )
    if m:
    ...
    if JSON_OUT.exists():
        try:
            ...
            if isinstance(existing, list):
                results = existing
                completed_services = {r["service"] for r in results}
                print(f"Resumed {len(results)} existing task entries.")
        except Exception as exc:
            print(f"Failed to load existing JSON: {exc}; starting fresh.")

    for task_no, svc, title in TASKS:
        if svc in completed_services:
            print(f"[{task_no}] Skipping {svc} (already verified).")
            continue
