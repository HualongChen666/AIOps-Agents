...
class StatisticalTests:
    ...
    def t_test(baseline: List[float], current: List[float], alpha: float = 0.05) -> Dict[str, Any]:
        ...
        try:
            ...
            effect_size = (
                abs(statistics.mean(current) - statistics.mean(baseline)) / pooled_std
                if pooled_std > 0
                else 0
            )
    ...
    def percentile_comparison(
        baseline: List[float], current: List[float], percentile: int = 95, threshold: float = 0.1
    ) -> Dict[str, Any]:
        """
        ...
        if not baseline or not current:
            return {"detected": False, "message": "Insufficient data for percentile comparison"}
        ...
        try:
            ...
            return {"detected": False, "message": f"Percentile comparison error: {str(e)}"}
...
class TrendAnalysis:
    ...
    def moving_average(values: List[float], window_size: int = 5) -> List[float]:
        ...
        return [
            statistics.mean(values[i : i + window_size])
...
class HistoricalDataManager:
    ...
    def _get_storage_file(self, test_name: str, metric_type: str) -> Path:
        ...
        return self.storage_path / f"{safe_name}_{metric_type}.json"
    ...
    def save_baseline(self, baseline: BaselineData) -> bool:
        ...
        try:
            with self._lock:
                ...
                # Set restrictive permissions for baseline file (644 - owner read/write, group/others read)
                try:
                    import os
                    import stat
    ...
    def load_baseline(self, test_name: str, metric_type: str) -> Optional[BaselineData]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to load baseline file {file_path}: {exc}")
                return None
...
class PerformanceRegressionDetector:
    ...
    def __init__(
        self,
        storage_path: Union[str, Path] = ".benchmarks/history",
    ...
    def detect_regression(
        ...
        for method in methods:
            result = self._run_detection_method(method, baseline.values, current_values, alpha)
    ...
    def _run_detection_method(
        self, method: DetectionMethod, baseline: List[float], current: List[float], alpha: float
    ) -> Dict[str, Any]:
        ...
        if method == DetectionMethod.T_TEST:
            ...
            return {
                ...
                "change_point": change_point,
                "message": (
                    f"Change point at index {change_point}"
                    if change_point
    ...
    def _calculate_severity(
        self,
        results: List[Tuple[DetectionMethod, Dict[str, Any]]],
        baseline: List[float],
        current: List[float],
    ) -> RegressionSeverity:
    ...
    def _trigger_alert(self, result: RegressionResult):
        ...
        with self._lock:
            if alert_key in self._alert_cooldowns:
                if now - self._alert_cooldowns[alert_key] < timedelta(
                    minutes=self.alert_config.cooldown_minutes
                ):
                    return
        ...
        if severity_order.index(result.severity) < severity_order.index(
            self.alert_config.severity_threshold
        ):
            return
    ...
    def analyze_trend(
        self, test_name: str, metric_type: str, values: List[float]
    ) -> Dict[str, Any]:
        """
...
class RegressionReportGenerator:
    ...
    def save_report(
        self, results: List[RegressionResult], output_path: Union[str, Path], format: str = "json"
    ) -> str:
        """
        Save report to file
