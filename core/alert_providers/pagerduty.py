...
class PagerDutyAlertProvider(AlertProvider):
    ...
    def _normalize_one(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        title = raw.get("title") or raw.get("summary") or raw.get("description") or "pagerduty-incident"
        ...
        started_at = raw.get("created_at") or raw.get("created") or datetime.now(timezone.utc).isoformat()
