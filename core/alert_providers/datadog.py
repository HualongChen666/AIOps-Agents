...
class DatadogAlertProvider(AlertProvider):
    ...
    def _normalize_one(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        event_type = str(raw.get("event_type") or raw.get("alert_transition") or raw.get("status") or "trigger").lower()
        status = "resolved" if event_type in ("recovery", "recovered", "ok", "resolved") else "firing"
        ...
        fingerprint = str(raw.get("alert_id") or raw.get("id") or raw.get("event_id") or uuid.uuid4().hex[:16])[:64]
        started_at = raw.get("date") or raw.get("date_event") or datetime.now(timezone.utc).isoformat()
