...
class LayoutCreate(BaseModel):
    ...
    breakpoints: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Responsive breakpoints")
...
async def list_components(
    ...
    try:
        ...
        if is_public is not None:
            filtered_components = [c for c in filtered_components if c.get("is_public") == is_public]
        if status:
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "translations": localization[request.language]
            },
            "timestamp": datetime.utcnow().isoformat()
        }
    except Exception as e:
