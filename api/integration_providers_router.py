...
async def test_gitops_connection(
    config_id: str = Path(..., description="Configuration ID")
) -> Dict[str, Any]:
    """
    测试GitOps配置的连接
...
async def test_cicd_connection(
    config_id: str = Path(..., description="Configuration ID")
) -> Dict[str, Any]:
    """
    测试CI/CD配置的连接
...
async def test_oncall_connection(
    config_id: str = Path(..., description="Configuration ID")
) -> Dict[str, Any]:
    """
    测试Oncall配置的连接
...
async def test_servicenow_connection(
    ...
    if config_id not in SERVICENOW_CONFIGS:
        raise HTTPException(
            status_code=404, detail=f"ServiceNow configuration {config_id} not found"
        )
...
async def test_message_queue_connection(
    config_id: str = Path(..., description="Configuration ID")
) -> Dict[str, Any]:
    """
    测试消息队列配置的连接
    ...
    if config_id not in MESSAGE_QUEUE_CONFIGS:
        raise HTTPException(
            status_code=404, detail=f"Message queue configuration {config_id} not found"
        )
...
async def test_elk_connection(
    ...
    if config_id not in ELK_CONFIGS:
        raise HTTPException(
            status_code=404, detail=f"ELK Stack configuration {config_id} not found"
        )
...
async def test_prometheus_connection(
    ...
    if config_id not in PROMETHEUS_CONFIGS:
        raise HTTPException(
            status_code=404, detail=f"Prometheus configuration {config_id} not found"
        )
