...
class RepairStrategyCreate(BaseModel):
    ...
    script_content: Optional[str] = Field(
        None, description="Script content for script-type repairs"
    )
...
class RepairExecutionUpdate(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="Execution status: pending, running, completed, failed"
    )
...
async def create_strategy(strategy: RepairStrategyCreate, request: Request) -> Dict[str, Any]:
    """
...
async def get_strategy(strategy_id: str) -> Dict[str, Any]:
    ...
    strategy_id = validate_path_param(strategy_id, "strategy_id")
...
async def update_strategy(
    strategy_id: str, strategy_update: RepairStrategyUpdate, request: Request
) -> Dict[str, Any]:
    """
...
async def create_execution(execution: RepairExecutionCreate, request: Request) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if strategy.get("auto_approve", False):
            ...
            try:
                ...
                logger.info(
                    f"Repair execution completed: {execution_id}, status={new_execution['status']}"
                )
            except Exception as e:
...
async def get_execution(execution_id: str) -> Dict[str, Any]:
    ...
    """
    # Validate path parameter
    execution_id = validate_path_param(execution_id, "execution_id")
...
async def update_execution(
    execution_id: str, execution_update: RepairExecutionUpdate, request: Request
) -> Dict[str, Any]:
    """
        ...
        if execution_update.status == "running" and existing.get("status") == "pending":
            ...
            try:
                ...
                if strategy_id in _repair_strategies:
                    ...
                    logger.info(
                        f"Repair execution completed: {execution_id}, status={existing['status']}"
                    )
            except Exception as e:
                logger.error(f"Repair execution failed: {e}", exc_info=True)
    ...
    try:
        ...
        default_platforms = [
            ...
            {"name": "Docker", "type": "docker", "capabilities": ["container", "image", "network"]},
            {
                "name": "Kubernetes",
                "type": "kubernetes",
                "capabilities": ["pod", "deployment", "service"],
            },
...
async def create_platform(platform: PlatformCreate, request: Request) -> Dict[str, Any]:
    """
...
async def get_platform(platform_id: str) -> Dict[str, Any]:
    ...
    """
    # Validate path parameter
    platform_id = validate_path_param(platform_id, "platform_id")
...
async def execute_cross_platform_repair(
    request_data: CrossPlatformRepairRequest, request: Request
) -> Dict[str, Any]:
    """
    Execute a repair across multiple platforms
    """
    logger.info(
        f"Executing cross-platform repair for {len(request_data.target_platforms)} platforms"
    )
    try:
        # Validate strategy exists
            ...
            for platform in request_data.target_platforms:
                try:
                    ...
                    result = await platform_strategy.execute_repair(
                        script_key, target_resource, request_data.parameters
                    )
            ...
            for platform in request_data.target_platforms:
                try:
                    ...
                    if not result.get("success"):
                        logger.warning(
                            f"Stopping sequential execution due to failure on {platform}"
                        )
                        break
                except Exception as e:
                    logger.error(f"Cross-platform repair failed for {platform}: {e}", exc_info=True)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to execute cross-platform repair: {str(e)}"
        )
...
async def create_template(template: RepairTemplateCreate, request: Request) -> Dict[str, Any]:
    """
...
async def get_template(template_id: str) -> Dict[str, Any]:
    ...
    """
    # Validate path parameter
    template_id = validate_path_param(template_id, "template_id")
...
async def update_template(
    template_id: str, template_update: RepairTemplateUpdate, request: Request
) -> Dict[str, Any]:
    """
...
async def delete_template(template_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        strategies_using = [
            s_id for s_id, s in _repair_strategies.items() if s.get("template_id") == template_id
        ]
        if strategies_using:
            raise HTTPException(
                status_code=409,
    ...
    try:
        ...
        if repair_type:
            executions = [
                e
                for e in executions
                if _repair_strategies.get(e.get("strategy_id"), {}).get("repair_type")
                == repair_type
            ]
        ...
        top_strategies = [
            ...
            for s_id, count in sorted(strategy_counts.items(), key=lambda x: x[1], reverse=True)[
                :10
            ]
        ]
...
async def create_unified_repair_alt(repair: Dict[str, Any], request: Request) -> Dict[str, Any]:
    """Alternative endpoint for frontend compatibility"""
...
async def execute_unified_repair_alt(repair_id: str, request: Request) -> Dict[str, Any]:
    """Alternative endpoint for frontend compatibility"""
...
async def export_repair_history_alt() -> Dict[str, Any]:
    ...
    for item in items:
        csv_data += (
            f"{item.get('id')},{item.get('strategy_id')},{item.get('status')},"
            f"{item.get('created_at')},{item.get('completed_at', '')}\n"
        )
...
@router_alt.post(
    "/scripts/executions/{execution_id}/cancel", summary="Cancel script execution (alt)"
)
...
async def update_repair_configuration_alt(
    config_id: str, updates: Dict[str, Any]
) -> Dict[str, Any]:
    """Alternative endpoint for frontend compatibility"""
...
async def get_hitl_approvals_alt(
    ...
    items = [e for e in _repair_executions.values() if e.get("status") == "pending"]
...
async def approve_hitl_request_alt(request_id: str, approval: Dict[str, Any]) -> Dict[str, Any]:
    """Alternative endpoint for frontend compatibility"""
...
async def reject_hitl_request_alt(request_id: str, rejection: Dict[str, Any]) -> Dict[str, Any]:
    """Alternative endpoint for frontend compatibility"""
    if request_id not in _repair_executions:
        raise HTTPException(status_code=404, detail="Request not found")
...
@router_alt.post(
    "/effectiveness/{effectiveness_id}/evaluate", summary="Evaluate effectiveness (alt)"
)
...
@router_alt.post(
    "/cross-platform/{repair_id}/execute", summary="Execute cross-platform repair (alt)"
)
...
async def list_scenarios_v1(
    ...
    return await list_strategies(
        repair_type=repair_type, platform=platform, status=status, priority=priority
    )
