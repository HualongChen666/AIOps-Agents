    ...
    try:
        ...
        for lang in _languages.values():
            if lang["code"] == language.code:
                raise HTTPException(status_code=400, detail=f"Language code '{language.code}' already exists")
...
@router.get("/languages/{language_id}", response_model=LanguageResponse, summary="Get a language by ID")
...
@router.patch("/languages/{language_id}", response_model=LanguageResponse, summary="Update a language")
...
async def create_resource(resource: ResourceCreate):
    ...
    try:
        ...
        for res in _resources.values():
            if (res["language_code"] == resource.language_code and 
                res["namespace"] == resource.namespace and 
                res["key"] == resource.key):
                raise HTTPException(
                    status_code=400, 
                    detail=f"Resource key '{resource.key}' already exists for language '{resource.language_code}' and namespace '{resource.namespace}'"
                )
...
@router.get("/resources/{resource_id}", response_model=ResourceResponse, summary="Get a resource by ID")
...
@router.patch("/resources/{resource_id}", response_model=ResourceResponse, summary="Update a resource")
...
@router.get("/translations", response_model=List[TranslationResponse], summary="Get all translations")
...
@router.post("/translations", response_model=TranslationResponse, summary="Create a new translation")
