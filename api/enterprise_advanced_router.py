...
class UserCreate(BaseModel):
    ...
    attributes: Optional[Dict[str, Any]] = Field(default_factory=dict, description="User attributes")
...
class PermissionCreate(BaseModel):
    ...
    permission_id: Optional[str] = Field(None, description="Permission ID (auto-generated if not provided)")
...
async def list_roles(
    ...
    try:
        ...
        if is_system_role is not None:
            filtered_roles = [r for r in filtered_roles if r.get("is_system_role") == is_system_role]
...
async def list_permissions(
    ...
    try:
        ...
        # Apply filters
        if resource:
            filtered_permissions = [p for p in filtered_permissions if p.get("resource") == resource]
        if action:
...
async def update_settings(request: SettingsUpdate) -> Dict[str, Any]:
    ...
    try:
        ...
        if ENTERPRISE_AVAILABLE:
            enterprise_functionality_manager.audit_retention_days = enterprise_settings.get("audit_retention_days", 90)
