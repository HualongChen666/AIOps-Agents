...
from core.hitl.approval import ApprovalWorkflow, ApprovalStep, ApprovalStatus
...
class RepairConfigCreate(BaseModel):
    ...
    config_type: str = Field(
        default="global",
        description="Configuration type: global, platform, resource, script"
    )
...
class RepairScriptCreate(BaseModel):
    ...
    language: str = Field(default="bash", description="Script language: bash, python, powershell, javascript")
...
async def create_effectiveness(effectiveness: EffectivenessCreate) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create effectiveness record: {str(e)}")
...
async def get_verifications(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch verification records: {str(e)}")
...
async def create_verification(verification: VerificationCreate) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create verification record: {str(e)}")
...
def _create_platform_repair_endpoint(
    ...
    @router.post(f"/{platform_name}/{{repair_id}}/repair", summary=f"Execute {platform_name} repair")
...
_get_hardware, _create_hardware, _execute_hardware = _create_platform_repair_endpoint(_hardware_repairs, "hardware")
_get_cloud, _create_cloud, _execute_cloud = _create_platform_repair_endpoint(_cloud_repairs, "cloud")
_get_cluster, _create_cluster, _execute_cluster = _create_platform_repair_endpoint(_cluster_repairs, "cluster")
...
_get_docker, _create_docker, _execute_docker = _create_platform_repair_endpoint(_docker_repairs, "docker")
_get_macos, _create_macos, _execute_macos = _create_platform_repair_endpoint(_macos_repairs, "macos")
_get_windows, _create_windows, _execute_windows = _create_platform_repair_endpoint(_windows_repairs, "windows")
_get_linux, _create_linux, _execute_linux = _create_platform_repair_endpoint(_linux_repairs, "linux")
...
async def get_cross_platform_repairs(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch cross-platform repairs: {str(e)}")
...
async def create_cross_platform_repair(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create cross-platform repair: {str(e)}")
...
async def create_unified_repair(
    repair: PlatformRepairCreate,
    request: Request
) -> Dict[str, Any]:
    """
...
async def update_script(
    script_id: str,
    script_update: RepairScriptUpdate
) -> Dict[str, Any]:
    """
...
async def get_intelligent_repairs(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch intelligent repairs: {str(e)}")


@router.post("/intelligent", summary="Create intelligent repair")
async def create_intelligent_repair(
    repair: IntelligentRepairCreate,
    request: Request
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create intelligent repair: {str(e)}")
...
async def analyze_intelligent_repair(repair_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to analyze intelligent repair: {str(e)}")
